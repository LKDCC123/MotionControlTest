...
//-------------------------------- global -----------------------------------------------
// blocks
const double nBlockPosi[3] = { 0.9, -0.32 - 0.02 - 10.0, 0.05 };
// push
#define __StartPushTime 6.0 
#define __PushDuration 0.1
const double pushtime[2] = { __StartPushTime, __StartPushTime + __PushDuration }; 
const double pushft[6] = { 0.0, -0.0 * 220.0, 0.0, 0.0, 0.0, 0.0 }; 

// camera
// #define AUTOCAMERA
const double lookat[3] = { 0.0, 0.0, 0.5 }; // position
const double distance = 5.0; // m
const double elevation = 10.0; // deg
const double azimuth = 185.0; // deg
...
void alignscale(void) // align and scale view
{
    ...
    #else
    // manualscale 20220921 dcc
    for(int i = 0; i < 3; i++) cam.lookat[i] = lookat[i];
            ...
            if( settings.run )
            {
                ...
                else
                {
                           ...
                           (glfwGetTime()-tmstart)<refreshfactor/vmode.refreshRate )
                    {
                        ...
                        fnvMjDataRecording();
                        // push
                        if(d->time > pushtime[0] && d->time < pushtime[1])
                        {
                            for(int i = 0; i <= 6; i++) d->xfrc_applied[12 + i] = pushft[i];
                        }
                        // run mj_step
