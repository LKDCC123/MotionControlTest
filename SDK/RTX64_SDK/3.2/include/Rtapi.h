...
// This function generates a custom monitoring event (an event of kind MF_EVENT_KIND_CUSTOM) having
// the specified custom kind and binary data.  Parameter "kind" specifies the kind of the custom
// event.  Parameter "size" specifies the number of bytes pointed to by parameter "data" that should
// be copied into the custom event.  Parameter "size" cannot be greater than 500.  If parameter
// "size" is non-zero, parameter "data" must point to a buffer containing "size" bytes of custom
// binary data, otherwise parameter "data" must be NULL.
//
// This function returns TRUE if successful or if monitoring is not started, FALSE otherwise.  When
// called from a real-time thread, if this function fails, GetLastError() will return one of the
// following values:
//
...
BOOL RtGenerateEvent(unsigned int kind, void * data, size_t size);

// MF_EVENT_KIND
// Each enumerator in this enumeration represents one kind of monitoring event.
typedef enum _MF_EVENT_KIND
{
    MF_EVENT_KIND_RESERVED = 0,     // This MUST be first and MUST have value 0!
